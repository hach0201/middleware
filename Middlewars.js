...
let demoLogger = (req, res, next) => {
    let current_datetime = new Date();              //get current time and date
    let formatted_date = 
    ...
    let method = req.method;       //request method
    let url = req.url;             //request url
    let status = req.statusCode;
    let log = `[${formatted_date}] ${method}: ${url} ${status}`;
    console.log(log);

    fs.appendFile("request_logs.txt", log + "\n", err => {      //append data to a file
        if (err){
            console.log(err);
        }
    });
...
app.post('/products', (req, res) => {
  ...
  app.use((error, req, res, next) => {
    res.status(error.status || 500);  // Définit le statut HTTP de la réponse en fonction de l'erreur
    res.json({
      error: {
        message: error.message  // Renvoie un objet JSON avec le message d'erreur
      }
    });
  });
  //port lisening
